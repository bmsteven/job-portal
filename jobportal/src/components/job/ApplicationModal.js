...
import {View, ScrollView, Text, TouchableWithoutFeedback, TouchableOpacity, Modal, Image} from "react-native"
...
const ApplicationModal = ({isVisible, closeModal, application, setApplication, job, logo}) => {
    console.log(application, job, logo);
                ...
                <View style={{
                    ...
                    backgroundColor: COLORS.white,
                        ...
                        <ScrollView style={{
                            ...
                            {/* render company */}
                            <Company name={company?.name} location={company?.location} logo={logo} />
                            ...
                            {/* render job name */}
                            <PrimaryHeader label={job?.name} containerStyle={{
                                    paddingHorizontal: 0,
                            ...
                            <View style={{
                                ...
                                {application?.accepted && <View>
                                    <SecondaryHeader label="Interview Schedule" showLine={true} customStyle={{
                                        marginTop: SIZES.radius
                                    }} />
                                    {application?.date && <View style={{
                                        flexDirection: "row",
                                        marginTop: SIZES.padding
                                    }}>
                                        <Text style={{
                                        ...
                                        <Text style={{
                                            ...FONTS.body3
                                        }}>{dayjs(application.date).format("MMM DD, YYYY HH:mm")}</Text>
                                    </View>}
                                    {application?.location && <View style={{
                                        flexDirection: "row",
                                        marginTop: SIZES.padding
                                    }}>
                                        <Text style={{
                            ...
                            {application?.pending && <View>
                                <SecondaryHeader label="The following items were sent to the employer" customStyle={{
                                    width: "100%",
                                }} />
                                <Text style={{
                                            ...FONTS.body3,
                                ...
                                {(user?.cv || user?.cvlink) && <Text style={{
                                            ...FONTS.body3,
                                            marginTop: SIZES.padding / 2
                                }}>· Your Resume</Text>}
                            </View>}
                        ...
                        <View style={{
                            ...
                            {application?.pending && <View style={{
                                flexDirection: "row",
                                justifyContent: "space-between",
                            }}>
                                <TextButton label="Close"  containerStyle={{
                                        marginHorizontal: 0,
                                ...
                                />
                                <TextButton label={loading ? "Please wait" : "Revoke application"}  containerStyle={{
                                        height: 45,
                                        flex: 2,
                                        marginVertical: SIZES.padding / 2,
                                        borderRadius: SIZES.radius / 2,
                                    ...
                                    onPress={() => {
                                        // revoke application
                                        revoke({id: job?.id, setApplication, setLoading, dispatch, closeModal})
                                    }}
                                />
                            </View>}
                            {application?.accepted && <View>
...
const Company = ({name, location, logo}) => {
    return <View>
        <View style={{
            flexDirection: "row",
            alignItems: "center",
